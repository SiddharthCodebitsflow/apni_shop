                ...
                if (data.status == 200) {
                    for (var i in data.data) {
                        if (Object.keys(data.data[i].relationship).length == 0) {
                            $('.card-box').append('<div class="col-md-4 col-sm-6 mt-4"><div class="card"><a href="/about-product/'+data.data[i].id+'"><img  src="' + data.data[i].product_image + '" class="product-image card-img-top" alt=""></a><div class="card-body"> <h5 class="card-title">' + data.data[i].product_name + '</h5> <div class="row"><div class="col-6"><p class="card-text mt-1"><s>Rs.' + data.data[i].regular_price + '</s></p></div><div class="col-6"><p class="card-text mt-1">Rs.' + data.data[i].vendor_price + '</p></div></div><p class="card-text"></p><div class="row"><a href="about-product/'+data.data[i].id+'" class="btn bg-custome text-white mt-1">Go to Detailed page</a></div> </div> </div> </div>');
                        } else {
